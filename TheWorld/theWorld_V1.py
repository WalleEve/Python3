...
def search_detail():
    ...
    # Label:
    capital = Label(dFrame, text="Capital: ")
    capital.grid(row=0, column=0, sticky=W, padx=5, pady=5)
    currency = Label(dFrame, text="Currency: ")
    currency.grid(row=1, column=0, sticky=W, padx=5, pady=5)
    language = Label(dFrame, text="Language: ")
    language.grid(row=2, column=0, sticky=W, padx=5, pady=5)
    population = Label(dFrame, text="Population: ")
    population.grid(row=3, column=0, sticky=W, padx=5, pady=5)



    # button
    exitButton = Button(dFrame, text="Exit")
    exitButton.grid(row=5, column=0)
    wikiButton = Button(dFrame, text="Wiki")
    wikiButton.grid(row=5, column=1)
...
mainFrame.place(x=5, y=50, width=390, height=140)

tileLabel = Label(tileFrame, text="Country Details", bg="aquamarine1", font=("Calibri", 15, "bold"))
tileLabel.pack()

searchBtn = Button(mainFrame, text="Search", font=("Calibri", 12, "bold"), command=search)
searchBtn.pack(side=LEFT,padx=10)

vlist = ["India", "China", "Japan",
          "Nepal", "Bhutan", "Saint Vincent and the Grenadines","Micronesia","Sao Tome and Principe","East Timor (Timor-Leste)"]
searchCombo = ttk.Combobox(mainFrame, values = vlist, font=("Calibri", 12, "bold"), textvariable = vSearch)
searchCombo.set("Pick an Option")
searchCombo.pack(side=LEFT, padx = 5, pady = 5)
# Notice
noticeFrame = Frame(mainFrame, bg="aqua")
noticeFrame.place(x=75, y=90, width=200, height=30)
